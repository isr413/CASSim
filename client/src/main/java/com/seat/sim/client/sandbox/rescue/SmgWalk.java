...
public class SmgWalk extends RescueScenario {
  ...
  private static class SmgTaskManager implements TaskManager {
    ...
    private void reportScore() {
      ...
      this.scenario.report(RescueScenario.MISSION_LENGTH, "Score: %.4f", 1. - weight);
    }
    ...
    public Optional<Zone> getNextTask(Snapshot snap, RemoteState state) {
      ...
      if (maxWeight.isEmpty()) {
        ...
        Zone choice = choices.get(this.scenario.getRng().getRandomNumber(3));
    ...
    public void update(Snapshot snap) {
      Stream<RemoteState> drones = snap
        ...
        .filter(state -> {
            int cooldown = this.scenario.getCooldown(state.getRemoteID());
            return cooldown == 0 || cooldown == SmgWalk.COOLDOWN;
          });
